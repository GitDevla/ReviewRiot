...
export class FeedModel {
    ...
    public static listFeed = async (user: UserModel, page: number, max: number) => {
        const res = await Database.query("SELECT review.id, review.rating, review.description, review.create_date, USER.name, USER.picture_path, movie.name as movie_name, movie.image_path FROM review JOIN `USER` ON review.author_id = USER.id JOIN movie ON review.movie_id = movie.id WHERE USER.id IN( SELECT whom_id FROM follow WHERE who_id = ?) OR USER.id = 1 ORDER by create_date limit ?,?;", user.id, page, max)
