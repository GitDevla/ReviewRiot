...
async function searchGetHandler(
    ...
    nameQuery = "%" + nameQuery + "%";
    const resp = await Database.query("SELECT id,name,picture_path as picture,'user' as type from `user` where name like ? UNION SELECT id,name,image_path,'movie' from `movie` where name like ? Limit 40;", nameQuery, nameQuery);
